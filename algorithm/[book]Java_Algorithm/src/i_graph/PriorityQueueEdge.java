...
public class PriorityQueueEdge<T> {
	PQNode<Edge<T>>[] nodes;
	int capacity;
	int usedSize;
	...
	public void pqEnqueueEdge(PQNode<Edge<T>> newNode) {
		...
		this.nodes[currentPosition] = new PQNode<Edge<T>>();
	...
	public void pqSwapNodes(int currentPosition, int parentPosition) {
		PQNode<Edge<T>> temp = new PQNode<Edge<T>>(); 
	...
	public PQNode<Edge<T>> pqDequeueEdge() {
		int parentPosition = 0;
		int leftPosition = 0;
		int rightPosition = 0;
		
		PQNode<Edge<T>> popped = this.nodes[0];
